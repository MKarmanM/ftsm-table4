...
export function CourseRow({
  ...
  const credit = Number(course.creditHours);
  const creditLabel = credit > 0 ? `${course.creditHours} kredit` : "Kredit belum dikira daripada SLT";
      ...
      <div className="rounded-md border border-border p-3">
        <form action={updateAction} className="space-y-2">
          ...
          <p className="rounded-md border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
            Nilai kredit tidak boleh diedit secara manual. Ia dikira automatik daripada jumlah SLT apabila Table 4 diterbitkan.
            {credit > 0 && <> Nilai semasa: <span className="font-medium text-foreground">{course.creditHours} kredit</span>.</>}
          </p>
          <select
            name="programmeId"
    ...
    <div className="flex items-center justify-between rounded-md border border-border px-3 py-2">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          {course.programmeCode} &middot; {creditLabel}
        </p>
      </div>
      <div className="flex items-center gap-2">
        <Button
